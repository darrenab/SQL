...
-- alter table actor add column nameCt int;

select last_name,count(last_name) as nameCt from actor
group by last_name;

-- #4b. List last names of actrs and the number of actors who have that last name, but only for names that are shared by at least two actors

select last_name, count(last_name) nameCt from actor
group by last_name
having nameCt > 1;
...
GROUP BY s.staff_id
;
-- #USE GREATER THAN, LESS THAN FOR FILTERING
...
#seems there's no column to join on
SELECT 
...
FROM
    store AS st
        JOIN
    address AS a ON a.address_id = st.address_id
        JOIN
    city AS c ON c.city_id = a.city_id
        JOIN
    country AS ct ON ct.country_id = c.country_id;
